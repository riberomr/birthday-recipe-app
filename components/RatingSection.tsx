...
interface RatingSectionProps {
    recipeId: string
    initialAverageRating?: { rating: number, count: number } | null
}

export function RatingSection({ recipeId, initialAverageRating }: RatingSectionProps) {
    const { profile } = useAuth()
    
    // Feature flags
    const showAverageRating = process.env.NEXT_PUBLIC_ENABLE_AVERAGE_RATING === 'true'
    const showUserRating = process.env.NEXT_PUBLIC_ENABLE_USER_RATING === 'true'

    const average = initialAverageRating?.rating || 0
    const count = initialAverageRating?.count || 0
